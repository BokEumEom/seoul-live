...
| CCTV 버튼, 요일×시간 히트맵 | 없음 | 1차 범위 밖 (아래 "의도적으로 뺀 것") |

**한글 폰트 주의:** 타이포 스케일은 시안대로지만 자형은 다르다. Hanken Grotesk에 한글 글리프가 없어서 한글은 기기 기본 폰트로 그려진다.
...
| 위치 훅이 화면 안에 있었음 | 상세에서 뒤로 갈 때마다 GPS를 새로 켬. 권한을 아직 안 정한 사용자에겐 팝업이 반복해서 뜸 | `LocationProvider`로 앱 수준에 한 번만 둠 |
