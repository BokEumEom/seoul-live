...
- **인과를 적을 때는 무엇으로 확인했는지 같은 자리에 적어라.** 주석과 커밋 메시지는 컴파일러도 테스트도 검사하지 않는 유일한 산출물이다. 코드가 맞는 채로 설명만 틀리면 **아무 신호가 없고** 다음 사람이 그 위에서 판단한다. 이 저장소에서 실제로 셋이 그랬다 — 「파생 불리언으로 감싸면 타입이 안 좁혀진다」(`tsc -b --force`는 0 errors였다), 「`preventScroll`이 없으면 시트가 튄다」(옵션 유무와 무관하게 200 그대로였다), 「마커를 누를 때 포커스가 검색창에 있으면 처방이 건너뛰어진다」(재현되지 않았다). 셋 다 결론은 맞고 이유가 틀렸다. **확인하지 않았으면 인과를 적지 말고 관찰만 적어라** — 「이 옵션을 넣어 뒀다」는 되고 「이 옵션이 없으면 튄다」는 안 된다.
