...
- **시트 손잡이의 히트 영역은 시트 루트 위로 20px 나가 있다.** 보이는 띠는 4px, 손에 닿는 상자는 44px(WCAG 2.5.8)이고 늘어난 몫을 위쪽에 몰아줬다. **시트 루트에 `overflow-hidden`을 걸지 마라** — 그 20px이 조용히 잘려 히트 영역이 24px로 돌아가고 테스트로도 안 잡힌다. 상단 요소가 둥근 모서리를 삐져나오거든 루트를 덮지 말고 그 요소를 깎아라.
