...
**모듈 최상위에서 `t()`를 부르지 마라.** import 시점의 언어로 굳어 언어를 바꿔도 그 자리만 옛말로 남는다. 상수 표는 한국어 키만 담고 쓰는 자리에서 감싼다.

**값이 낀 문구는 `{이름}` 자리를 쓴다** — `t('주차 {비율}%', { 비율: 45 })`. 통째로 키를 삼으면 값마다 새 키가 되고, 영어에서 어순이 달라질 때 못 바꾼다(`{비율}% parking free`).

**`src/i18n/i18n.test.ts`가 셋을 잠근다:** 사전 완결성(요청한 키가 다 있나), 역방향(안 쓰는 항목이 남았나), **섞임 방지**(화면에 `t()` 안 감싼 한국어가 있나). 마지막 것이 「절반만 번역된 화면」을 막는 자리다.

**번역할 수 없는 것 셋:** 서울 API의 자유 문장(`message`·재난문자), 고유명사(주차장·대여소·행사 이름), 명소 이름. 영어 화면에도 한국어가 남는 것은 결함이 아니다. 도로소통 지수(`원활`·`서행`·`정체`)도 뺐다 — 값의 종류를 모르고, `원활`이 혼잡도 헤드라인과 **뜻이 다른 동음이의**라 키가 다툰다.
