...
- **바텀시트는 진입 시 열려 있으면 안 된다.** 앱인토스 심사 항목이다("미니앱에 진입하자마자 바텀시트가 자동으로 나타나지 않아요"). 시안 `stitch_ui/_1`은 열린 채로 그려져 있으니 그대로 옮기지 마라. **현재 코드에는 바텀시트가 없다** — 2026-08-07 개편으로 `AreaSheet`가 없어지고 명소 요약이 목록 자리의 인플레이스 상세로 바뀌었다. 시트를 다시 넣을 일이 생기면 이 조항이 그대로 적용된다.

- **화면은 셋이고 지도가 홈이다.** `HomeScreen`(지도+목록+상세) / `FavoritesScreen` / `TodayScreen`. 「내 주변」과 「혼잡예보」를 탭으로 되살리지 마라 — 둘 다 갈 곳이 아니라 지도에 대고 하는 일이라서 각각 버튼·정렬 기준과 상세로 녹였다. 근거는 `docs/superpowers/specs/2026-08-07-map-home-redesign-design.md`.

- **홈은 언제나 마운트된 채로 둔다.** `App.tsx`가 `<div hidden={tab !== 'home'}>`으로 감싼다. 조건부 렌더로 바꾸면 탭을 오갈 때마다 `google.maps.Map`이 다시 생성돼 타일을 다시 받고 카메라·검색어·분할 비율이 날아간다.
...
| `stitch_ui/` | 화면 시안 4종 + 디자인 토큰. 시안은 4탭 기준이라 현재 구조(3탭)와 다르다 — 토큰만 정본이다 |
