...
//
// **`citydata-bulk.ts` 자체는 2026-08-27에 지웠다(Task 7, 죽은 일괄 조회 경로).**
// 그 시점에 이미 그 프록시를 부르는 클라이언트 코드가 하나도 안 남아 있었다 —
// `MAX_AREAS`(40)도 그 파일과 함께 저장소에서 사라졌다. 그래서 지금 이 파일이
// 지키는 것은 카탈로그 크기가 아니라 **허용 목록이 카탈로그와 같은 집합인가**뿐이다.
