...
// **주석을 걷어내고 센다.** 이 저장소는 필드 이름을 주석에 자주 적는다 —
// 「`SUB_ARMG2`는 읽지 않는다」, 「구간 목록은 일부러 안 읽는다(`LINK_ID`·
// `ROAD_NM`·`SPD`·`XYLIST`)」처럼 **안 읽는다고 적은 것**이 특히 그렇다.
// 주석까지 세면 그 여섯이 「구현」으로 뒤집혀, 트래커가 정확히 반대로 말한다
// (2026-08-25에 실제로 그 상태였다: 주석 포함 117 vs 실제 111).
...
>
> **명세에 없는 것이 응답에는 있다.** 2026-08-25 실호출에서 \`CHARGER_STTS\`의 \`CHARGER_DETAILS\`, \`LIVE_CMRCL_STTS\`의 \`CMRCL_RSB\`, \`ROAD_TRAFFIC_STTS\`의 \`XYLIST\`가 확인됐다 — 명세는 그 안쪽 필드를 한 겹 펼쳐 적어 놓고 배열 이름을 안 적었다. **명세의 순번만 보고 구조를 짐작하지 마라.**
>
> 원문(탭 구분)이 갱신되면 \`node scripts/format-citydata-spec.mjs docs/citydata-spec-raw.tsv\`로 다시 만든다. 「구현」 열도 그때 다시 센다.
...
**5. \`ROAD_TRAFFIC_IDX\`의 값 목록이 명세에 없다.** 「원활」·「서행」·「정체」가 명세 전문에서 0회다. **그 사실은 그대로이고, 2026-08-21에 색을 붙였다** — 미룰 때 걱정한 것은 「처음 보는 값에 **틀린 색**이 붙는 것」이었는데, \`roadIndexTone\`이 아는 셋만 매핑하고 나머지는 \`null\`이라 그 일이 일어나지 않는다. 색이 안 붙을 뿐 틀리지 않는다.

셋을 아는 근거는 명세가 아니라 **실호출 응답**이다: \`docs/fixtures/citydata-광화문덕수궁.json\`에 전체 지표 \`정체\`, 구간 지표 \`정체\`·\`서행\`이 있다. \`원활\`은 사전(\`i18n/en.ts\`)에 이미 있었다. 문자열 자체는 여전히 **그대로 보여준다** — 아는 값만 그리면 처음 보는 값에서 지표가 사라진다.

**7. 이 파일을 손으로 고치지 마라.** \`scripts/format-citydata-spec.mjs\`가 통째로 찍어 내는 생성물이다. 손으로 고친 절은 다음 재생성에서 **말없이 사라진다** — 2026-08-25에 실제로 위 5번 문단을 그렇게 잃었다가 되살렸다. 고칠 것이 있으면 스크립트의 템플릿을 고쳐라.
