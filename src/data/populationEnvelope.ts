 ...
 *
 * **`CITYDATA`가 있으면 그 자리에서 확정한다 — 빈 객체라도, `LIVE_PPLTN_STTS`가
 * 없어도 옛 봉투로 안 내려간다.** 배타적 우선순위다. 2026-08-27 대조에서 두
 * 봉투가 한 payload에 같이 온 적이 없어서 안전했을 뿐, **이 순서 자체가 그
 * 전제 위에 서 있다.** 옛 갈래를 지울 때(마지막 태스크) 이 사실을 아는 채로
 * 지워야 한다 — 지금도 이미 `CITYDATA`가 있으면 옛 갈래는 한 번도 안 읽힌다.
 *
