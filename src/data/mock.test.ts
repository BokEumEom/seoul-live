...
describe('buildMockSnapshot', () => {
  ...
  it('명소마다 혼잡도가 다르게 나온다', () => {
    const levels = new Set(
      AREA_CATALOG.map(
        (a) => parseCitydataResponse(buildMockSnapshot(a.name), a.name).congestion,
      ),
    )
  ...
  it('카탈로그의 모든 명소에 대해 동작한다', () => {
    for (const area of AREA_CATALOG) {
      expect(() =>
        parseCitydataResponse(buildMockSnapshot(area.name), area.name),
      ).not.toThrow()
    }
  })
  ...
  describe('자정을 넘기는 시각', () => {
    ...
    it('23시 기준으로 만든 예측도 다음날로 굴러가며 스키마를 통과한다', () => {
      const snapshot = parseCitydataResponse(buildMockSnapshot('강남역'), '강남역')
