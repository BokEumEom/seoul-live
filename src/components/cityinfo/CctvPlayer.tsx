    ...
    void (async () => {
      try {
        ...
        const hls = new Hls({
          ...
          enableWorker: true,
          // **서울시 것(15초)보다 길게 잡았다.** 그 값을 그대로 쓰면 거의
          // 언제나 타임아웃이다 — 실측(2026-08-19, 3회)에서 2.4MB 세그먼트
          // 하나가 서울시 프록시로 **17.5 / 17.0 / 19.2초** 걸렸다. 15초는
          // 그 아래라 첫 조각부터 `fragLoadTimeOut`으로 죽는다.
          //
          // **이건 응급 처치지 해결이 아니다.** 10초짜리 조각을 받는 데 18초가
          // 걸리면 재생은 원리상 실시간을 못 따라간다. 같은 조각을 원본
          // (UTIC)에서 직접 받으면 **0.4초**다 — 병목은 우리도 원본도 아니고
          // 서울시 프록시다. 제대로 고치려면 스트림도 우리 프록시로 날라야
          // 하는데 그건 영상 대역폭을 우리가 지는 결정이라 사람이 정할 일이다.
