 ...
 * 막대 길이가 같은 말을 한다.
 *
 * `AGE_LABELS`와 길이가 묶여 있다 — 구간이 늘면 칸이 모자라 색 없는 막대가
 * 조용히 생긴다. 타입으로는 못 막아서 아래 `rankClass`가 끝 값으로 떨어뜨리고
 * `AgeShareRows.test.tsx`의 「연령 구간이 늘어도」가 잠근다.
 *
...
interface Props {
  /** 연령대별 백분율. `AGE_LABELS`와 차례가 같다 */
  readonly rates: readonly number[]
}

/**
 * 연령대별 비율. 시안 `stitch_ui_ux/_3`의 「연령대별 비율」이다.
 *
 * ```
 * 10대  [▓▓░░░░░░░░]  15%
 * 20대  [▓▓▓▓▓▓░░░░]  35%
 * ```
 *
 * **한 막대에 여덟 칸을 쌓던 것을 여덟 줄로 폈다.** 쌓은 막대는 전체가 100%가
 ...
 * 그 자체로 정보이기 때문이다.
 */
export function AgeShareRows({ rates }: Props) {
  ...
  return (
    ...
    <ul role="list" className="mt-3 flex flex-col gap-2">
      {visible.map((index) => (
        ...
        // 같은 줄을 지웠다 새로 만든다.
        <li key={AGE_LABELS[index]} className="flex items-center gap-2">
