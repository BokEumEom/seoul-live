...
//
// 드래그는 손잡이에서만 받는다. 내용 영역에서도 받으면 시트 안을 스크롤할
// 때마다 시트가 따라 내려간다. 손잡이는 끌 수도 있고 누를 수도 있다 —
  ...
  return (
    ...
    >
      ...
      {/* **여기에 padding을 걸지 마라.** 시트에 들어오는 뷰 셋(목록·상세·
          ...
          `data-sheet-content`는 **지우면 안 되는 계약**이다. 이 상자가 시트의
          스크롤 컨테이너이고, `HomeScreen`이 뷰를 갈아 끼울 때 이 표식으로
          찾아 `scrollTop`을 0으로 되돌린다 — 세 뷰가 이 컨테이너 하나를
          나눠 쓰기 때문이다. 표식이 없어지면 새 뷰가 앞 뷰의 스크롤 자리에서
          시작하고, jsdom에는 레이아웃이 없어 그 회귀를 잡을 길이 없다.

          테스트 손잡이이기도 하다. 자식 텍스트에서
