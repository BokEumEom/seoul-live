...
//
// **다만 「과거를 못 받는다」는 공식 API에 한정된 말이다**(2026-08-27). 인증키를
// 안 쓰는 SeoulRtd 쪽은 과거를 준다 — `/api/ppltn`의 1시간·3시간·한달 전 대비는
// 이미 붙였고(`domain/populationTrend.ts`), `/api/ppltn_congest`는 **과거 12시간
// 실측 + 현재 + 예보 12시간(25칸)**과 함께 `before_people_value`(같은 25칸의
// **과거 4주 평균**)를 준다. 이 표를 기기에 쌓는 대신 그쪽에서 받는 길이 열려
// 있다는 뜻이다. 아직 안 했다 — 이 파일은 그대로다.
