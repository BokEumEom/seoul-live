...
/**
 * 도로소통이 쓰는 키(`ROAD_SEGMENT_INDEXES`). **접두어가 없는 맨 값이다.**
 *
 * 예전에는 절이 「도로 원활」로, 요약 카드가 「원활」로 서로 다른 키를 썼다.
 * 접두어를 만들게 했던 충돌 상대(혼잡도 헤드라인의 「원활」)는 2026-08-20에
 * 사라졌고, 2026-08-25에 절이 배지로 바뀌면서 절 이름(「도로소통」) 바로
 * 아래에 「도로 정체」를 적게 되어 접두어가 되레 같은 말을 두 번 하게 됐다.
