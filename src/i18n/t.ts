...
// ---
//
// **모듈 최상위에서 `t()`를 부르지 마라 — 상수 표는 함수여야 한다.**
//
// 모듈 최상위의 `t()`는 **import 시점에 한 번만** 불린다. 그 시점의 언어로
// 값이 굳어, 나중에 언어를 바꾸고 트리를 다시 그려도 그 자리만 한국어로
// 남는다. 위의 「앱 루트가 구독해 다시 그린다」가 통하지 않는 유일한 자리다.
//
//     const LABELS = { light: t('밝게') }            // 굳는다
//     function labels() { return { light: t('밝게') } }   // 매번 새로 읽는다
//
// **예시의 문자열도 사전에 있는 것을 써라.** 아래 완결성 검사는 소스를 훑어
// `t()`에 넘긴 문자열을 모으는데 **주석 안까지 센다.** 없는 문자열을 예시로
// 적으면 유령 번역 요청이 되어 검사가 죽는다 — 실제로 이 주석을 쓰다 한 번
// 죽였다(2026-08-21, 지워진 정렬 줄의 「여유한 순」을 예시로 썼다).
//
// **사전 검사로는 안 잡힌다.** 완결성 검사는 「`t()`에 넘긴 문자열이 사전에
// 있는가」를 보는데, 이 결함은 사전에 항목이 **있는데도** 화면에 한국어가
// 뜨는 것이라 전부 초록이다. 2026-08-18에 실제로 **6개 파일 19개 문자열**이
// 그 상태였다 — 정렬 줄·화면 테마·지도 앱 버튼(KakaoMap·Naver·TMAP)·위치
// 안내·평소 대비 문구·지도 실패 안내.
//
// 그래서 `i18n.test.ts`의 「모듈 최상위에서 t()를 부르지 않는다」가 파일을
// 파싱해 막는다. 사람이 지키는 규칙이었던 동안 위 19개가 생겼다.
//
// 이 저장소에서 이 규칙을 따르는 자리들: `LocationNotice`·`MapLinkButtons`·
// `PopulationLead`·`MapUnavailableNotice`·`ThemeSetting`. 전부 상수 표처럼
// 생겼지만 함수다.
