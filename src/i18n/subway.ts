 ...
 * 없다」였다. 그 전제는 틀렸다(2026-08-25) — 실호출 응답의 `SUB_DIR`에
 * `상행`·`하행`이 둘 다 있다. 그래서 이 집합은 **행선지 규칙에서만** 빼고
 * 번역은 사전에 맡긴다. 역 이름과 달리 방향어는 로마자 표기를 지어내는 일이
 * 아니라 낱말을 옮기는 일이다.
 *
 * **다만 이 갈래는 실제로 거의 안 밟힌다.** 파서는 `SUB_DIR`을 아예 안 읽고
 * `SUB_TERMINAL`로 「대화행」을 만든다(`cityInfoSchema.ts`의 `directionOf`) —
 * 실호출 아홉 편 전부 종착역이 있었다. 여기 오는 것은 종착역도 노선명도 비어
 * 호출부가 `SUB_DIR`을 넘기게 되는 날뿐이다. **화면에서 「상행」을 봤다는
 * 관찰이 아니라 방어다.**
 */
