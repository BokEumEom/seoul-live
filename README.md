...
| 탭 | 내용 |
| --- | --- |
| **지도** | Google Maps 위 혼잡도 마커 + 명소 목록. 검색·목적 프리셋·카테고리·정렬. 손잡이로 지도와 목록 비율을 조절한다. 명소를 누르면 지도는 그대로 두고 아래만 상세로 바뀐다 — 시간별 예측, 길찾기·공유, 접이식 도시 정보(주차장·따릉이·날씨·문화행사·재난문자) |
| **즐겨찾기** | 담아둔 명소만. 이 기기에 저장된다 |
| **더보기** | 오늘의 서울 — 혼잡도 분포, 붐빔·여유 TOP, 카테고리별 평균, 재난문자 모음, 여유로운 곳 추천 |
...
src/components/   props만 받는다
                  home/ 홈 전용(분할·검색·상세) · list/ 목록 · today/ 오늘의 서울
                  cityinfo/ 도시 정보 · map/ 지도 · forecast/ 예측 · common/ · layout/
src/screens/      화면 조립 — HomeScreen · FavoritesScreen · TodayScreen
