...
| **명소 상세** | 시간별 예측 차트, 길찾기·저장·공유, 인구 구성(성별·연령·상주비율), 접이식 도시 정보(주차장·따릉이·날씨·문화행사·재난문자) |
...
5. `CITYINFO_CACHE_TTL_SECONDS`를 정한다 — 명소 상세의 접이식 도시 정보 전용 TTL. 비워두면 위 값으로 떨어진다. 도시 정보는 `citydata`를 따로 부르므로 혼잡도와 하루 한도를 나눠 쓴다([AGENTS.md](./AGENTS.md)의 계산 참고)
