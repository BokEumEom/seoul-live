...
4. `CACHE_TTL_SECONDS`를 설정한다 — 갤러리 등록 전 `3600`, 후 `300`. **하루 1,000회를 쓰는 손잡이는 이것 하나다.** 목록·지도·CCTV·인파 변화는 인증키를 안 쓰는 상류라 0회이고, 쿼터를 쓰는 것은 사용자가 실제로 연 **상세**뿐이다. `3600`이면 하루에 서로 다른 명소 41곳까지 들어온다(24 × 41 = 984). [AGENTS.md](./AGENTS.md)의 쿼터 조항 참고
5. `CITYINFO_CACHE_TTL_SECONDS`는 **보통 비워 둔다** — 비우면 위 값과 같아진다. 2026-08-27에 혼잡도와 도시 정보를 한 응답(`citydata`)으로 합치면서 「도시 정보만 따로 부르는 호출」이 없어졌고, 그래서 둘을 다른 값으로 둘 이유도 함께 사라졌다
