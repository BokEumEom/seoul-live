...
> **아래 코드는 1차 초안이다.** 코드 리뷰 후 신뢰 경계를 강화하면서 다음이 추가됐다. 실제 구현은 `src/data/schema.ts`를 보라.
>
> - `parseCitydataResponse(payload, expectedName)` — 인자가 둘이다. **위 권위 규칙을 prose로만 적고 코드에 안 넣어서** 검증이 통째로 빠져 있었다. `sample` 키가 뭘 요청하든 광화문을 돌려주는 걸 앱이 못 잡는 상태였다.
> - `AreaNameMismatchError` — 요청한 명소가 응답에 없을 때. 카탈로그 오타를 첫 호출에서 터뜨린다.
> - `SeoulApiError` — 서울 API의 `RESULT` 봉투(`INFO-200 해당하는 데이터가 없습니다`)를 보존한다. 오타 진단에 가장 쓸모 있는 정보였는데 형태 불평으로 대체되고 있었다. `responseSchema` 파싱이 **실패했을 때만** 시도해 성공 응답을 오탐하지 않는다.
> - `numericSchema`가 `z.string().regex(/^\d+$/)` — `Number('')`이 `0`이라 빈 값이 "추정 인구 0~0명"으로 통과했다. `-500`, `0x1f`, `1e5`도 막는다.
> - `PPLTN_TIME`도 시각 스키마를 탄다. `observedAtLabel`(`"14:35"`)을 함께 노출해 화면이 원본 형식을 파싱하지 않게 한다.
> - 시각 정규식이 범위까지 검사한다 — `(?:[01]\d|2[0-3]):[0-5]\d`. 느슨한 `\d{2}`는 `"99:99"`를 통과시켜 "99시엔 여유 예상"이 뜬다.
> - 시각 파싱이 zod 파이프라인 안으로 들어갔다. 전부 경로가 붙은 하나의 `ZodError`로 나간다.
> - `min <= max` refine.
...
export async function fetchAreaSnapshot(areaName: string): Promise<AreaSnapshot> {
  ...
  return parseCitydataResponse(await requestJson(url), areaName)
}
...
export async function fetchAreaSnapshots(
  ...
  if (useMock()) {
    return areaNames.map((name) =>
      parseCitydataResponse(buildMockSnapshot(name), name),
    )
  }
...
1. ~~**일괄 조회 가능 여부 확인**~~ — **불가능으로 확정.** 공식 데이터셋 문서가 "한 번에 1개 장소씩만 호출 가능"이라고 명시한다. `api/citydata-bulk.ts`가 명소별로 병렬 호출하는 현재 구조가 유일한 방법이고, 서버 측 공용 캐시가 필수다.
