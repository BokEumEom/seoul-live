...
import { congestionRank, isCalm } from '../domain/congestion'
  ...
  return list
    .filter(
      (item) =>
        ...
        item.snapshot !== null &&
        isCalm(item.snapshot.congestion),
    )
...
function forecast(time: string, congestion: Forecast['congestion']): Forecast {
  return { time, congestion, populationMin: 0, populationMax: 0 }
}
...
export function findQuietTime(
  ...
  const hour = Number(quiet.time.slice(11, 13))
  return Number.isFinite(hour) ? hour : null
}

export function forecastHour(forecast: Forecast): number {
  return Number(forecast.time.slice(11, 13))
}
```
...
import { forecastHour } from '../../domain/forecast'
    ...
    <div>
      ...
      <div className="mt-1 flex justify-between text-xs text-on-surface-variant">
        {labelIndexes.map((index) => (
          <span key={index}>{forecastHour(forecasts[index])}시</span>
        ))}
      </div>
    </div>
  )
