...
function requestSheetFocus(): void { setFocusRequest((count) => count + 1) }
useEffect(() => {
  if (focusRequest === 0) return          // 0 = 아직 아무도 요청하지 않았다
...
**`document.activeElement`가 입력인지 보는 길은 택하지 않았다.** 그건 원인이 아니라 **증상**을 보기 때문이다. 타이핑은 그것으로도 고쳐지지만 **칩 경로는 못 고친다** — 칩을 눌러 선택이 풀릴 때는 손이 칩 줄에 있는데(연달아 다른 칩을 누르려던 참이다) 칩은 입력이 아니라서 포커스를 그대로 뺏긴다. 리뷰어가 옛 코드에 그 가드를 얹고 칩 경로를 걸어 **도난을 재현했다.**

**한때 근거로 적었던 두 번째 항목은 거짓이었다 — 지웠다.** 「마커로 상세를 여는 순간 포커스가 아직 검색창에 남아 있으면 처방이 통째로 건너뛰어진다」고 썼는데, 이 화면에서는 일어나지 않는다. 탐침으로 확인했다: 검색창에 포커스를 두고 `fireEvent.click`(포커스를 안 옮긴다)으로 마커를 눌러도, `openArea`의 `setDetent('full')`이 같은 커밋에서 검색 바를 언마운트해 **effect 시점의 `activeElement`는 이미 `BODY`**다. 입력이 살아 있는 채로 뷰가 갈리는 경로가 이 화면 구조상 없다. **근거 (2) 하나로 결론이 서므로 결정 자체는 그대로다.**
...
### 포커스 처방이 만든 **두 번째** 결함 — 요청 신호가 굳는다 (R1)

첫 구현은 요청을 `useRef`로 들었다. **ref는 렌더를 일으키지 않으므로 전달을 보장하지 못한다.**

경로: 상세를 연다 → 시트 위로 남은 지도 조각에서 **같은 마커**를 다시 누른다.
`openArea`의 세 setter가 전부 같은 값이라(`selectedName` 동일, `view === 'list'`, `detent === 'full'`) React가 렌더를 건너뛴다 → effect도 안 돈다 → **신호만 세워진 채 굳는다.** 그다음 아무 렌더(지도 팬·시트 조작·창 복귀 refetch — `QueryProvider`가 `refetchOnWindowFocus: true`다)에서 포커스가 시트로 튄다.

| 시점 | `activeElement` |
| --- | --- |
| 1회차 마커 클릭 후 | 시트 상자 (정상) |
| 2회차 마커 클릭 후 | 마커 버튼 — 렌더 없음, 신호 굳음 |
| 그 뒤 카메라 변경 후 | **시트 상자 (도난)** |

**Critical은 아니다** — C1의 타이핑 유실은 안 되살아난다. 검색 바를 꺼내려면 `detent`를 바꿔야 하고 그 자체가 렌더라 신호가 거기서 소비된다. 실제 피해는 엉뚱한 순간의 포커스 이동 한 번이고, 눈에는 안 보이지만 키보드·스위치·스크린리더 사용자는 맥락을 잃는다.

**고친 방식: 요청을 ref가 아니라 카운터 상태로 든다.** 증가값은 언제나 새 값이라 「요청했으면 반드시 한 번 돈다」가 보장되고, 의존성 배열도 자연스럽게 돌아온다. **불리언 상태로는 안 된다** — 이미 참인데 참을 넣으면 React가 또 건너뛴다. 변이로 확인했다(R1b: 불리언으로 바꾸면 「뷰가 갈려도 포커스가 시트를 벗어나지 않는다」가 죽는다).

정상 경로에서 렌더가 늘지 않는다. `openArea`가 이미 setter 셋을 같은 배치에서 부르므로 넷째가 붙어도 렌더는 한 번이다. 늘어나는 것은 **바로 그 bail-out 경우 하나**인데, 그때 포커스가 시트로 가는 것은 옳다 — 사용자가 그 명소로 가겠다고 누른 것이다. 카운터가 넘칠 일은 없다(`Number.MAX_SAFE_INTEGER`).

**교훈: ref는 「렌더 사이에 값을 나른다」이지 「신호를 전달한다」가 아니다.** 소비자가 effect라면 그 effect가 도는 것까지가 전달인데, ref는 그걸 못 만든다.

### 포커스 처방이 만든 **세 번째** 결함 — 시트 스크롤이 안 돌아온다 (I-1)

`data-sheet-content`는 목록·상세·오늘의 서울이 **함께 쓰는 스크롤 컨테이너 하나**인데, 태스크마다 뷰를 하나씩 맡다 보니 **컨테이너의 주인이 아무도 아니었다.** 뷰가 갈려도 `scrollTop`이 그대로라 새 뷰가 앞 뷰의 스크롤 자리에서 시작한다.

실측(390×844) — 고치기 전 / 고친 뒤:

| 동선 | 「목록으로」 | 요약 스트립 | 맨 위에 보이던 것 |
| --- | --- | --- | --- |
| 목록 200 → 상세 | `top −47.5` (밖) → **`91.5`** | — | 액션 행 → **「목록으로」** |
| 상세 → 목록 | — | 잘림 → **`top 479.8`** | 목록 행 → **요약 스트립** |

**두 번째가 특히 나쁘다** — 「오늘의 서울」로 가는 유일한 통로가 요약 스트립인데, 상세를 한 번 다녀오면 그게 스크롤 밖이라 입구가 사라진 것처럼 보인다.

`requestSheetFocus()`를 부르는 네 곳이 정확히 「뷰가 갈리는」 네 경우라 조건을 새로 만들 필요가 없었다. 컨테이너는 **`parentElement`가 아니라 `closest('[data-sheet-content]')`로** 집는다 — 사이에 상자가 하나만 끼어도 조용히 엉뚱한 요소를 만지고, 그 회귀는 jsdom이 못 잡는다(Task 9에서 `.parentElement`로 같은 함정을 밟았다). 그래서 `data-sheet-content`는 이제 테스트 손잡이가 아니라 **런타임 계약**이고, `BottomSheet` 주석에 그렇게 적었다.

**jsdom은 「잘렸다」를 못 잡지만 `scrollTop = 0` 대입은 보존한다** — 확인하고 그걸로 잠갔다(변이 S1).
...
| R1 | 포커스 요청을 카운터 상태 → ref 플래그 (**R1 회귀**) | 1개 — 같은 명소를 다시 눌러도 포커스 요청이 남지 않는다 |
| R1b | 카운터 → **불리언** 상태 | 1개 — 시트 안에서 뷰가 갈려도 포커스가 시트를 벗어나지 않는다 (불리언도 같은 병을 앓는다는 실증) |
| S1 | 시트 스크롤 되돌리기 제거 (**I-1 회귀**) | 1개 |
| S2 | `closest('[data-sheet-content]')` → `parentElement` | **0개 — 생존.** 아래 |
| S3 | `favorites.length` → `counts.fav` (시트 올림 조건) | 1개 — 담아둔 게 있으면 지금 0이어도 시트를 올리지 않는다 |
| S4 | `openArea`의 `setView('list')` 제거 | 1개 — 오늘의 서울에서 연 상세를 닫으면 오늘의 서울이 아니라 목록이다 |
| S5 | 손잡이 세로 히트 확장(`-mt-5 pt-7.5 pb-2.5`) 제거 | 1개 |
| S6 | 시트 루트에 `overflow-hidden` 추가 | 1개 |
| S7 | `walkableMinutes` → `walkingMinutes` (도보 상한 제거) | 1개 |
| S8 | 목록 뷰의 `sr-only` h2 제거 | 1개 |
...
**살아남은 변이 3건 — 전부 「테스트를 더 쓸 자리가 아니다」.** (M18은 목록에서 빠졌다 — `viewKey`가 통째로 사라져 변이시킬 코드가 없다.)

**S2는 오늘의 DOM에서 두 표현이 같은 요소를 가리키기 때문에 생존한다.** 지금은 그 상자가 스크롤 컨테이너의 첫 자식이라 `parentElement`도 맞는 답을 준다 — 갈리는 것은 사이에 상자가 끼는 **미래**뿐이고, 테스트로는 그 미래를 만들 수 없다(컴포넌트 트리를 밖에서 못 바꾼다). M18과 같은 「구성으로 막는 것」 부류다.
...
- **`RecommendationCard`가 `walkingMinutes`(상한 없음)를 쓰던 것 → `walkableMinutes`(60분 상한)로 바꿨다.** `AreaHero`는 이미 후자를 쓴다. 지금은 `pickRecommendations`가 반경 2km를 막아 두 함수의 결과가 같지만, `domain/distance.ts`가 「상한을 화면마다 따로 두면 같은 값이 자리마다 다르게 잘린다 — 판정은 도메인이 갖는다」고 적어 두고 정작 한 화면이 그걸 안 지키고 있었다. 반경이 늘어나는 날 `AreaHero`는 그대로인데 여기만 「도보 160분」을 적게 된다 — 죽은 가지가 아니라 **그 도메인 주석이 말하는 방어선**이다. 없던 `RecommendationCard.test.tsx`를 만들어 두 경우를 잠갔다.
- **목록 뷰에 `sr-only` h2 「명소 목록」을 넣었다.** 상세는 히어로 h2, 오늘의 서울은 절 제목을 갖는데 **기본 화면인 목록만** `App`의 h1 아래가 비어 제목으로 훑는 사용자에게 빈 칸이었다. 이름이 가리키는 것은 목록 자체다 — 위의 요약 줄·필터·정렬까지 덮는 이름을 붙이면 실제 구조와 어긋난다.
- **`AGENTS.md`의 「`src/components/`는 props만 받는다」를 사실대로 다시 썼다.** 이미 세 번 깨져 있었고(`AreaDetail`·`CityInfoPanel`, 그리고 이 브랜치의 `NearbyCalmSection`) **규칙이 새 위반을 낳는 채로 두면 안 된다.** 진짜 경계는 「화면이냐」가 아니라 **「그 데이터를 쓰는 곳이 자기 하나뿐이냐」**다. 셋 다 상세 아래의 자립적인 절이고 형제도 부모도 그 질의를 안 본다 — 끌어올리면 `AreaDetail`이 자기가 안 쓰는 값을 내려보내기만 하는 통로가 된다. 대가(테스트가 `data/queries`를 목업해야 한다)와 한계(두 곳 이상이 같은 데이터를 보면 끌어올릴 신호)를 함께 적었다.
- **`AGENTS.md`의 손잡이 조항에서 「테스트로도 안 잡는다」를 갈랐다.** `overflow-hidden`이 잘라내는 **높이 자체**는 jsdom이 못 재지만, 그 값을 만드는 클래스(`-mt-5 pt-7.5 pb-2.5`)와 루트에 `overflow-hidden`이 없다는 것은 **둘 다 잠글 수 있었다.** 잠갔다(변이 S5·S6).
- **`console.error` 4겹(지도 로드 실패)은 그대로 뒀다.** StrictMode 이중 렌더 + `APIProvider`가 겹친 것이고 StrictMode는 개발 빌드에서만 돈다. `setLoadFailed`가 멱등이라 화면은 정상이고, 스테일 클로저를 피해 가며 로그만 줄이는 가드는 얻는 것보다 읽기 비용이 크다.
...
- `npm test` — **625개 통과 + 1 todo** (Task 9의 612 + 1에서 순변화 **+13**: 지운 8개, 새로 쓴 21개)
  - 파일별(실행 기준): `FavoritesScreen` 6 → 0 / `HomeScreen` 38 → 52 / `FilterChips` 16 → 17(`it.each` 둘이 4건씩 펼쳐진다) / `App` 12 → 12 / `BottomSheet` 31 → 33 / `RecommendationCard` 0 → 2(새 파일)
- `npx tsc -b --force` / `npm run lint` / `npm run build:vite` — 전부 통과
- 변이 **42건 중 39건 사망**, 생존 3건은 jsdom이 못 보는 기하이거나 구성으로만 막히는 것이다

**「콘솔 에러 0건」 완료 기준 — 실측하고 해석을 남긴다.** 정상 경로(명소 열기 → 저장 → 공유 → 목록 → 오늘의 서울 → 목록 → 손잡이 3단 → 내 장소 칩)를 걸어 센 결과: **`console.error` 1건, `console.warn` 0건, 미처리 예외 0건.** 그 1건은 `NotAllowedError: Failed to execute 'writeText' on 'Clipboard': Document is not focused` — **헤드리스라 클립보드가 막힌 것**이고, 앱은 실제로 일어난 실패를 옳게 적은 것이다(`platform/links.ts`의 공유 폴백). 앱 결함이 아니므로 기준을 통과한 것으로 본다.
...
- **새 미해결 항목** (대부분 실기기로만 확인된다. 마지막 하나는 제품 결정이다):
  ...
  - **내용과 함께 DOM에 삽입되는 live region을 일부 보조기술이 놓친다** — 빈 목록 안내는 상자 자체가 조건부 렌더라 `role="status"`가 내용과 동시에 나타난다. 표준 완화책은 리전을 **상시 렌더**하고 안쪽 텍스트만 조건부로 두는 것이다. 확인할 수단이 없어 결함이 아니라 미해결로 남긴다 — 실기기에서 낭독을 못 들으면 이것부터 의심하라 (Task 10)
