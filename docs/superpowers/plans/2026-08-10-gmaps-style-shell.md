...
**(C) `z-10` 시트와 `z-20` 검색 바가 맞닿는다.** 아래 Step 3의 검색 바·필터 칩이 `z-20`이라 시트 위에 뜬다. `full`(92%)에서 시트 상단은 뷰포트의 8%이고(800px 기준 64px) 검색 바는 top 0에서 약 56px를 먹는다 — 거의 붙는다. 실기기에서 겹치는지 확인하고, 겹치면 `full`을 낮추든 검색 바를 `full`에서 숨기든 정해라.
