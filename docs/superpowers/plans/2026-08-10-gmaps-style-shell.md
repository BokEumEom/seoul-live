...
- `npm test` — **617개 통과 + 1 todo** (Task 9의 612 + 1에서 순변화 **+5**: 지운 8개, 새로 쓴 13개)
  - 파일별(실행 기준): `FavoritesScreen` 6 → 0 / `HomeScreen` 38 → 48 / `FilterChips` 16 → 17(`it.each` 둘이 4건씩 펼쳐진다) / `App` 12 → 12. 합 72 → 77
- `npx tsc -b --force` / `npm run lint` / `npm run build:vite` — 전부 통과
- 변이 **34건 중 32건 사망**, 생존 2건은 jsdom이 못 보는 기하다
