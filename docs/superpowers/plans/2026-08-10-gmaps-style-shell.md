...
- `npm test` — **613개 통과 + 1 todo** (Task 9의 612 + 1에서 순변화 **+1**: 지운 8개, 새로 쓴 9개)
  - 파일별(실행 기준): `FavoritesScreen` 6 → 0 / `HomeScreen` 38 → 44 / `FilterChips` 16 → 17(`it.each` 둘이 4건씩 펼쳐진다) / `App` 12 → 12. 합 72 → 73
- `npx tsc -b --force` / `npm run lint` / `npm run build:vite` — 전부 통과
- 변이 **28건 중 25건 사망**, 생존 3건은 전부 jsdom이 못 보는 기하이거나 표현 불가능한 충돌
